...
selectInput("y", 
            label = tags$strong("Seleccionar Rubro", align = "center"),
            choices =  sort(names(indec[ ,!(colnames(indec) %in%  column_indexes)])), 
            selected = "salud")

span(textOutput("message"), style="color:black; font-size: 22px; text-align:center; font-family:math; align-self:center;")

output$message <- renderText({
    paste0(
      "Evolucion de los precios en Argentina desde enero de 2017 hasta ",
      format(max(indec$periodos), "%B"),
      " de ", as.character(max(indec$year)))
  })
...
renderHighchart({
  ...
  highcharter::hchart(
    ...
    hc_legend(enabled = FALSE) %>% 
      ...
      highcharter::hc_caption(
          text = paste0("Grafico de barras de ",str_to_title(gsub("_", " ", input$y)),". <br>Fuente: Indec.")) %>%
      hc_title(
        text = paste0('Inflacion ', gsub('_', ' ', colnames(indec[,c(input$y)]))),
        style = list(fontSize = '16px', fontWeight = 'bold')) %>% 
      hc_subtitle(
...
**Dashboard sobre los precios en Argentina**

Este Dashboard tiene la finalidad de mostrar la variación mensual e interanual de los distintos sectores de la economía para la región GBA.

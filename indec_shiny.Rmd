...
#indec = indec[,c(3,8,46,47,48)]

#nombres = colnames(indec[,c(1,2)])
...
selectInput("y", "Y-Axis", choices = sort(names(indec[,-c(14,15,16)])), selected = "Frutas")
...
renderValueBox({
  valueBox(value = tags$p(paste0(deframe(indec[,c(input$y)])[nrow(indec)], ' %'), style = "font-size: 100%; color: #8a002c"), 
           icon = "fa-diamond",
           caption = tags$p(paste0('Inflacion Mensual de ',input$y), style = "font-size: 100%; color: #8a002c"),
           color = '#f5d3dd')
})
